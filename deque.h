...
public:
	...
	iterator end() const;
	const_iterator cbegin() const;//metodo introdotto dal C++11
	const_iterator cend() const;//metodo introdotto dal C++11
	void push_front(const K&);
	void push_back(const K&);
	K pop_front();
	K pop_back();
	void erase(iterator);
	void deleteAt(int);//indico l'indice da eleminare
};

//----------------------------------------CLASSE NODO
template<class K>
deque<K>::nodo::nodo(): info(), next(0), prev(0) {}

template<class K>
deque<K>::nodo::nodo(nodo* p) {this=p;}

template<class K>
deque<K>::nodo::nodo(const K& data, nodo* precedente, nodo* successivo): info(data), next(successivo), prev(precedente) {}
//----------------------------------------------------
...
void deque<K>::insertAt(int pos,K inf){//revisionare
    if(pos<0 || pos>size()) throw OutIndex();
	nodo* a=inizio;
	for(int i=0; i<pos; ++i){
		a=a->next;
	}
	if(empty()){
		inizio=fine=new nodo(inf,0,0);
		return;
	}
	if(inizio==fine){
		if(pos==0){
			inizio=new nodo(inf,0,fine);
			fine->prev=inizio;
			return;
		}else{
			fine=new nodo(inf,inizio,0);
			inizio->next=fine;
			return;
		}
	}
	if(a==inizio){//controllare utilità
		inizio=new nodo(inf,0,a);
		a->prev=inizio;
		return;
	}
	if(a==0){
		fine->next=new nodo(inf,fine,0);
		fine=fine->next;
		return;
	}
...
void deque<K>::deleteAt(int i){
    if(i<0 || i>=size()) throw (error("deleteAt(): indice fuori range"));
	int x=0;
	nodo* a=inizio;
	while(x<i){
		a=a->next;
...
template<class K>
void deque<K>::erase(iterator it){
    nodo* a = it.ptr;
    if(it.ptr->next)
